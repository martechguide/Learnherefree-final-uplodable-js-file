...
export class DatabaseStorage implements IStorage {
  ...
  async authenticateAdmin(username: string, password: string): Promise<User | null> {
    ...
    // Admin credentials hardcoded for security (fallback)
    if ((username === "admin" && password === "admin123") || 
        (username === "Satya1251" && password === "Golu@917008")) {
      console.log('Admin hardcoded credentials matched');
    ...
    // Check if the username is an email or hardcoded username
