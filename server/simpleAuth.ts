...
export function setupSimpleAuth(app: Express) {
  ...
  app.use(session({
    secret: process.env.SESSION_SECRET || 'replit-session-secret-development-' + Math.random().toString(36),
    store: sessionStore,
    resave: false,
    ...
    try {
      ...
      if (!user) {
        ...
        const newUser = {
          ...
          lastName: '',
          role: 'user', // Default role - admins can be configured in database
