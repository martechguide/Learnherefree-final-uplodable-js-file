...
<body>
  ...
  // Email service test route (for development only)
    ...
    try {
      ...
      res.json({
        ...
        message: connectionTest ? 
          `Email service (${emailStatus.provider}) is working properly` : 
          `Email service (${emailStatus.provider}) connection failed`
      });
    } catch (error) {
      res.status(500).json({ 
  ...
  // Forgot password route - Simple OTP system
  app.post('/api/auth/forgot-password', async (req, res) => {
    try {
      ...
      if (emailSent) {
        // Email sent successfully - never show OTP directly
        res.json({ 
          message: "Password reset OTP has been sent to your email address. Please check your email and enter the 6-digit code.",
          email: email,
          expiresIn: "10 minutes",
          success: true
        });
      } else {
        // Temporary development fallback - show OTP in response for testing
        console.log(`DEVELOPMENT FALLBACK: OTP for ${email} is: ${otp}`);
        res.json({ 
          message: "Email service configuration needed. For development: OTP is displayed in server logs.",
          email: email,
          expiresIn: "10 minutes",
          success: true,
...
<body>
    <div class="login-container">
        ...
        <div class="admin-note">
            Create an admin account and set role to 'admin' in database
        </div>
